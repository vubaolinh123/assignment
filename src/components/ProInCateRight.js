...
const ProInCateRight = {
	async printf(id) {
		const { data } = await getAll();
		const ProInCate = await Relationships(id)
		const dataProInCate = ProInCate.data.products;

		return /*html*/ `
			<div class="tittle-sp bg-red-500 text-white text-xl py-3 px-5 text-center"><span class="">Sản Phẩm Thuộc Danh Mục ${ProInCate.data.name}</span>
				</div>
				<div class="py-3 grid grid-cols-3 gap-3">
						${dataProInCate.map((product) => {
			if (product.status == 1) {
				var status = "Còn Hàng"
			} else {
				status = "Hết Hàng"
